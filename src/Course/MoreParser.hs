...
tok ::
  Parser a
  -> Parser a
tok p = p >>= \x -> (spaces >> valueParser x)
...
between ::
  ...
  -> Parser a
  -> Parser a
between op cp xp = do op; x <- xp; cp; return x
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok o c px = between (charTok o) (charTok c) px
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
-- sepby1 x s = x >>= \x' -> list (s >> x) >>= (\xs' -> return (x':.xs'))
sepby1 xp sp = do x <- xp; xs <- list (sp >> xp); return (x:.xs)
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby x s = sepby1 x s ||| return Nil
