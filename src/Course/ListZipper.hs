...
setFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
setFocus x lz = withFocus (const x) lz
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper l c r) = case break p l of
  (_, Nil) -> IsNotZ
  (u, s :. ss) -> IsZ $ ListZipper ss s (reverse u ++ c :. r)
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper l c r) = case break p r of
  (_, Nil) -> IsNotZ
  (u, s :. ss) -> IsZ $ ListZipper (reverse u ++ c :. l) s ss
...
moveLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveLeftN n lz | n >= 0 = (P.iterate (moveLeft -<<) (IsZ lz)) P.!! n
...
moveRightN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveRightN n lz | n >= 0 = (P.iterate (moveRight -<<) (IsZ lz)) P.!! n
...
moveLeftN' n lz = moveLeftN'' n lz 0
  where moveLeftN'' n' lz' c | n' == 0 = Right lz'
                             | n' < 0 = moveRightN' (-n') lz'
                             | otherwise = case moveLeft lz' of
                                             IsNotZ -> Left c
                                             IsZ lz'' -> moveLeftN'' (n' - 1) lz'' (c + 1)
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n lz = moveRightN'' n lz 0
  where moveRightN'' n' lz' c | n' == 0 = Right lz'
                              | n' < 0 = moveLeftN' (- n') lz'
                              | otherwise = case moveRight lz' of
                                              IsNotZ -> Left c
                                              IsZ lz'' -> moveRightN'' (n' - 1) lz'' (c + 1)
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth i lz@(ListZipper l _ _) = moveLeftN (length l - i) lz
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper lf cf rf) (ListZipper lx cx rx) = ListZipper (zipWith ($) lf lx) (cf cx) (zipWith ($) rf rx)
...
instance Extend ListZipper where
  (<<=) f lz
    = ListZipper
        (unfoldr (\lz' -> (\lz'' -> (f lz'', lz'')) <$> (toOptional . moveLeft $ lz')) lz)
        (f lz)
        (unfoldr (\lz' -> (\lz'' -> (f lz'', lz'')) <$> (toOptional . moveRight $ lz')) lz)
...
instance Traversable ListZipper where
  traverse f (ListZipper l c r) = ListZipper <$> (traverse f l) <*> f c <*> (traverse f r)
