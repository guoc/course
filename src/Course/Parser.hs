...
valueParser ::
  a
  -> Parser a
valueParser x = P (\inp -> Result inp x)
...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f (P p) = P (\inp -> let (Result r v) = p inp in Result r (f v))
...
(|||) px py = P $
  \inp -> case parse px inp of
            ErrorResult _ -> parse py inp
            Result r v    -> Result r v
...
list1 ::
  Parser a
  -> Parser (List a)
list1 p = flbindParser p (\v -> flbindParser (list p) (\vs -> valueParser (v:.vs)))
...
sequenceParser =
  foldRight
    (\p acc -> p `flbindParser` (\pv -> acc `flbindParser` (\accv -> valueParser (pv :. accv))))
    (valueParser Nil)
...
firstNameParser ::
  Parser Chars
firstNameParser = upper `flbindParser` (\u -> list lower `flbindParser` (\ls -> valueParser (u:.ls)))
...
surnameParser ::
  Parser Chars
surnameParser =
  upper `flbindParser` (\u -> thisMany 5 lower `flbindParser` (\ls -> list lower `flbindParser` (\ls' -> valueParser (u:.ls ++ ls'))))
...
phoneParser ::
  Parser Chars
phoneParser = digit `flbindParser` \d -> phoneBodyParser `flbindParser` \b -> is '#' >>> valueParser (d:.b)
...
personParser ::
  Parser Person
personParser =
  ageParser `flbindParser` \a -> spaces1 >>>
  firstNameParser `flbindParser` \fn -> spaces1 >>>
  surnameParser `flbindParser` \sn -> spaces1 >>>
  smokerParser `flbindParser` \s -> spaces1 >>>
  phoneParser `flbindParser` \p ->
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  (<$>) g p = p `flbindParser` \x -> valueParser (g x)
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) pg px = pg `flbindParser` \g -> px `flbindParser` \x -> valueParser (g x)
