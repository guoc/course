...
-- Fail to pass infinity testing
-- map f = foldLeft (\acc -> \x -> acc ++ (f x :. Nil)) Nil
...
-- seqOptional (ox :. oxs) | ox == Empty = Empty -- Why does this not work?
seqOptional (Empty :. _) = Empty
seqOptional (Full x :. oxs) = P.fmap (x :.) (seqOptional oxs)
...
lengthGT4 xs = lengthGT 4 xs
  where lengthGT i Nil = i < 0
        lengthGT i (_ :. ys) | i <= 0 = True
                             | otherwise = lengthGT (i - 1) ys
