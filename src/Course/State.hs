...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  (<$>) f (State g) = State (\s -> let (x, s') = g s in (f x, s'))
...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b 
  (<*>) (State f) (State g) = State (\s -> let (h, s') = f s in let (x, s'') = g s' in (h x, s''))
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  (=<<) k (State g) = State (\s -> let (x', s') = g s in runState (k x') s')
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat xs = fst $ runState (findM (\x -> State (\s -> (x `S.member` s, S.insert x s))) xs) S.empty
...
distinct ::
  ...
  List a
  -> List a
distinct xs = eval (filtering (\x -> State (\s -> (x `S.notMember` s, S.insert x s))) xs) S.empty
...
isHappy ::
  Integer
  -> Bool
isHappy x = let Full r = firstRepeat (1 :. produce square x) in if r == 1 then True else False
  
toDigits ::
  Integer
  -> List Integer
toDigits x = x `mod` 10 :. (if q == 0 then Nil else toDigits q)
  where q = x `div` 10

square ::
  Integer
  -> Integer
square x = foldRight (+) 0 (P.fmap (P.^2) . toDigits $ x)
