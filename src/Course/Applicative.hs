...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List b
  (<*>) Nil _ = Nil
  (<*>) fs xs = flatMap (\f -> P.fmap f xs) fs
...
sequence ::
  ...
  -> f (List a)
sequence =
--   foldRight (\x -> \acc -> pure (:.) <*> x <*> acc) (pure Nil)
  foldRight (\x -> lift2 (:.) x) (pure Nil)
...
filtering ::
  ...
  -> List a
  -> f (List a)
-- filtering p = foldRight (\x -> \acc ->  ((\b -> \l -> if b then (x :.l) else l) <$> (p x) <*> acc) ) (pure Nil)
